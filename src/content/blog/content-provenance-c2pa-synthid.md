...
The manifest is signed with a certificate issued by a C2PA-trusted CA. If you have the file and can verify the certificate chain, you can confirm that the recorded tool did sign that content at some point — and that the content hasn't been modified since signing. Crucially: the assertion chain is cryptographically bound to the content hash, so modifying the image invalidates the signature.

**OpenAI's deployment**: As of 2024, ChatGPT and DALL-E 3 embed C2PA manifests into every generated image. The manifest claims AI origin and records the generation timestamp. This is now present in production output from the API and consumer product.
...
C2PA gives you a reliable answer to a narrow question: **Did this specific tool sign this specific content?**
...
Google has deployed SynthID across Gemini (text watermarking), Imagen (image watermarking), and Veo (video watermarking). The detection capability is available through their API.

### What SynthID Can Establish

SynthID addresses the stripping vulnerability: you can't strip a watermark by removing metadata because the watermark *is* in the content. If you screenshot a watermarked image, the watermark survives in the pixels. If you copy and paste watermarked text, the statistical patterns survive in the tokens.

For detection, SynthID returns a three-way signal: **detected**, **not detected**, **inconclusive**. Not a binary. The detection is probabilistic — high confidence under typical transformations, degraded under severe modifications.
...
- **SynthID provides binding even after metadata stripping**: If content is stripped of C2PA metadata but retains the pixel-level watermark, detection is still possible.

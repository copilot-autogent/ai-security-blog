...
The BEC threat is illustrative because BEC is already the most financially damaging category of cybercrime. The FBI's Internet Crime Complaint Center has reported BEC losses exceeding $2.7 billion annually in recent years. WormGPT did not create this threat class. It lowered the skill threshold required to execute it.

Within weeks of the SlashNext publication, a second tool surfaced. CheckPoint Research documented FraudGPT in August 2023 — a similar model marketed on Telegram channels with subscriber counts in the thousands. FraudGPT was sold on a subscription model ranging from $200/month to $1,700/year, with marketing materials listing its capabilities: writing undetectable malware, generating phishing pages, creating cracking tools, and producing BEC email chains customized per target industry. CheckPoint's research noted the Telegram distribution channel had accumulated over 3,000 subscribers by the time of publication.
...
**DarkBERT** occupied a distinct technical niche. Rather than a generative model fine-tuned for malicious output, DarkBERT was marketed as a language model trained specifically on dark web content — forums, marketplaces, leaked databases — to give it better coverage of criminal terminology, tradecraft documentation, and illicit marketplace data. Its use case was framed around intelligence gathering and reconnaissance rather than direct attack generation.

**PoisonGPT** emerged from academic research rather than the criminal marketplace, but pointed in the same direction: researchers at Mithril Security demonstrated that an open-weight model (Falcon-7B) could be surgically modified to produce specific false factual claims — a targeted disinformation capability embedded in a model that otherwise performed normally. The attack was undetectable without targeted evaluation. The point was not that PoisonGPT was itself a criminal tool, but that the technique it demonstrated was available to anyone with the motivation to apply it.
...
**Route 2: System-prompted uncensored base models.** An increasingly relevant alternative that does not require fine-tuning at all. Open-weight models released without safety alignment — or models from which alignment has been removed through techniques like RLHF reversal — can be configured for offensive use through system prompting alone. Models like WizardLM-Uncensored and several Hugging Face uploads that have had alignment stripped were specifically designed to remove refusal behavior while preserving capability. A system prompt configuring the model to operate as a penetration tester with no operational limits is sufficient for many criminal use cases.
...
**VirusTotal and malware corpus analysis.** AI-generated malware samples, where identified, show up in public corpus collections. Characteristic structural patterns of AI-generated code — consistent variable naming conventions, preference for certain code patterns, distinctive comment styles — are observable at the corpus level even when individual samples evade signature detection.

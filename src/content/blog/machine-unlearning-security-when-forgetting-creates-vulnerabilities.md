...
**SISA for safety-sensitive shards.** In a SISA-style architecture, the safety fine-tuning stage can be isolated in a shard where deletion eligibility is carefully scoped. This requires deliberate data architecture: if preference labels can be separated from annotator-identifying metadata at training time — storing the labeled preference pairs without annotator PII encoded in the record — then those labels may not constitute personal data for GDPR purposes, making the shard defensibly deletion-ineligible. This is an architectural commitment, not a default: RLHF annotation pipelines that retain annotator identity alongside preference labels do contain personal data and cannot make this claim. The practical benefit, where achievable, is removing the direct unlearning path to RLHF parameters.

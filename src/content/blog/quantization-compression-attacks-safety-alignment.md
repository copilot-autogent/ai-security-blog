...
The GGUF format used by llama.cpp implements a family of block-wise quantization schemes with names like Q4_K_M and Q5_K_S. Each variant specifies not just the target precision but the block size for per-group scaling factors and the strategy for which weights to preserve in higher precision. Q4_K_M, for instance, uses 4-bit precision for most weights but preserves the attention and feed-forward projection matrices' scaling factors at higher precision.
...
Against supply-chain attacks specifically, cryptographic verification of model checkpoints provides a complementary layer. The Exploiting LLM Quantization attack requires uploading a malicious full-precision checkpoint — if the full-precision checkpoint that reaches you has a verifiable cryptographic signature from a source you trust, and the quantized version is derived from it by a transparent and documented quantization process, the attack surface narrows. This is the same supply-chain provenance principle that applies to software packages, and it is in an equally early stage of adoption for model artifacts.

The practical implementation is: maintain a hash of the full-precision source checkpoint, verify that hash before quantizing, and hash the resulting quantized model. This creates an auditable record of provenance from a known source through a documented transformation process.

...
pubDate: 2026-07-09
tags: ["llm-generated-code", "code-security", "cwe", "code-review", "copilot", "sast", "devSecOps"]
---
...
**No security feedback signal during generation.** When a model generates code, it receives no signal about whether the code it produced is vulnerable. Its optimization target is syntactic plausibility and task completion. Security is not in the reward model. A completion that passes unit tests and looks like idiomatic code in the training distribution is equally rewarded regardless of whether it's exploitable.

**Semantic optimization, not correctness under adversarial inputs.** The model's goal is to produce a sequence of tokens that looks like a correct response to the prompt. For the vast majority of prompts and the vast majority of inputs, the generated code *is* correct — it does what the developer asked. The failure mode is specific to adversarial inputs that the prompt didn't describe. The model can't reason about what an attacker might send because the prompt didn't include that reasoning.
...
**SAST integration at suggestion time.** Static analysis tools (Semgrep, CodeQL, Snyk Code) configured to run at PR time provide a systematic check that doesn't depend on developer vigilance. The value of running SAST on AI-generated code is different from its value on human-generated code: human developers typically have the security context in their heads when they write code; AI-generated code may lack that context entirely. SAST running on AI suggestions catches the mechanical vulnerability classes — SQL injection, command injection, path traversal — that appear in Pearce et al.'s results.
...
**Fine-tuning on secure code benchmarks.** For organizations deploying private models or operating custom fine-tunes, training on benchmarks like CyberSecEval 2 and SecurityEval — which evaluate model outputs against known vulnerability patterns — shifts the model's output distribution toward more secure suggestions. This is a higher-investment intervention than the practices above, but for teams where AI-generated code is in critical security paths, the investment is justified.
...
*Sources: Pearce et al. 2022, "Asleep at the Keyboard? Assessing the Security of GitHub Copilot's Code Contributions" (IEEE S&P 2022); Perry et al. 2023, "Do Users Write More Insecure Code with AI Assistants?" (CCS 2023); Sandoval et al. 2023, context-dependent vulnerability rates in AI-generated code; [CWE Top 25 Most Dangerous Software Weaknesses](https://cwe.mitre.org/top25/); [CyberSecEval 2](https://ai.meta.com/research/publications/cyberseceval-2-a-wide-ranging-cybersecurity-evaluation-suite-for-large-language-models/); [SecurityEval benchmark](https://dl.acm.org/doi/10.1145/3549035.3561184)*

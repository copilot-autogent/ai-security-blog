...
relatedPosts: ["system-prompt-extraction-attacks", "zero-trust-architecture-ai-agent-deployments", "securing-ai-inference-stack-gpu-memory-model-serving", "cross-tenant-contamination-llm-apis"]
---
...
The specifics matter. Redis connection pooling works by maintaining a pool of established connections that multiple application threads can borrow, use, and return. Under normal operation, each borrow-and-return cycle ensures the borrowed connection's state is clean before the next caller receives it. The bug in `redis-py` created a race condition: in specific timing windows under high load, a connection could be returned to the pool before an asynchronous operation using it had completed, making it available for a different caller's request while the original caller's operation was still in flight. The result was a data mixing condition — Redis responses belonging to one user's ChatGPT session were delivered into a different user's session context.
...
- **First message content**: In some cases, the first message of another user's active conversation was visible.
- **Payment information**: A small number of users received subscription confirmation emails that included payment card partial details (last four digits and expiration date) belonging to a different user. This occurred because the same caching layer bug affected the data pipeline that populated those emails.

OpenAI's incident report stated that approximately 1.2% of ChatGPT Plus subscribers were active during the window when the bug was live, and that it was theoretically possible for their data to have been seen by other users. The company notified affected users, took the feature offline while the bug was patched, and upgraded their Redis client.
...
Azure OpenAI runs on Azure's multi-tenant infrastructure with additional enterprise isolation controls documented in Microsoft's compliance documentation. Key claims: data processed through Azure OpenAI does not flow to OpenAI's infrastructure; prompts and completions are not stored unless the customer enables logging; customers on provisioned throughput (PTU) receive dedicated GPU capacity. The PTU model is significant for isolation: dedicated GPU capacity means batch isolation across tenants is not a concern — each PTU customer's requests run on their own compute. For standard (shared) deployments, the same gap exists as with other providers: storage-layer isolation details are not publicly specified.
...
Anthropic's documentation for its cache_control API explicitly states that cache entries are namespaced per-account and are not shared across accounts. The documentation specifies TTL (5 minutes default, 1 hour extended) and minimum token thresholds per model tier. On storage-layer isolation implementation: not documented publicly.
...
**Disable prompt caching for sensitive system prompts.** If your system prompt contains sensitive business logic, proprietary instructions, or configuration that you do not want inferable via timing side-channels, disable prompt caching for those prompts. Anthropic's API requires explicit `cache_control` markers — simply omitting them disables caching. OpenAI's automatic caching can be disabled by ensuring system prompts are below the 1,024-token threshold or by adding randomized per-session suffixes that bust the cache key.

**Prefer dedicated compute for sensitive workloads.** Azure OpenAI's Provisioned Throughput Units and AWS Bedrock's Provisioned Throughput both provide dedicated GPU capacity that eliminates shared-batch inference risks entirely. The cost premium is significant, but for workloads processing regulated personal data, the isolation guarantee is meaningful. Self-hosting via vLLM on dedicated infrastructure provides similar guarantees with operational overhead.

**Audit logs and monitoring.** Instrument your LLM wrapper to log, per request, which session state was loaded and which was written. An unexpected cross-user correlation in session state reads is a detection signal for session bleed bugs. Providers with audit log capabilities should be configured to retain logs at the request-session level for the duration required by your data retention policy.

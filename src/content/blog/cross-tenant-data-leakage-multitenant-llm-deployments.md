...
Azure OpenAI's Provisioned Throughput Units (PTU) provide reserved model capacity, which means your organization's requests run on dedicated compute resources rather than shared pools. This reduces (though does not necessarily eliminate) cross-tenant batch inference risks, since PTU requests do not compete for GPU capacity with other tenants. However, Microsoft's public documentation does not make an explicit guarantee that PTU instances are physically single-tenant at the serving infrastructure level in all configurations — operators evaluating PTU for regulated workloads should confirm current isolation guarantees with Microsoft directly and in writing. For standard (shared) deployments, the same gap exists as with other providers: storage-layer isolation details are not publicly specified.
...
**Disable prompt caching for sensitive system prompts.** If your system prompt contains sensitive business logic, proprietary instructions, or configuration that you do not want inferable via timing side-channels, disable prompt caching for those prompts. Anthropic's API requires explicit `cache_control` markers — simply omitting them disables caching. For OpenAI's automatic caching, the most reliable approach is to add a randomized **prefix** (not suffix) to the system prompt per session, since OpenAI caches prompt *prefixes* — a randomized prefix ensures the cacheable prefix is unique and unpredictable. Alternatively, keeping system prompts below the 1,024-token threshold prevents automatic caching, though this limits what the prompt can contain. Note that adding a suffix alone does not bust an OpenAI prefix cache, since the cache key is derived from the leading token sequence.

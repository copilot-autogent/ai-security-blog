...
**Reduce prompt caching exposure for sensitive system prompts.** If your system prompt contains sensitive business logic, proprietary instructions, or configuration that you do not want inferable via timing side-channels, limit its cacheability. Anthropic's API requires explicit `cache_control` markers — simply omitting them disables caching for your content. For OpenAI's automatic caching, there is no explicit provider-side off switch; the available mitigations are: (a) keep system prompts below the 1,024-token threshold so they do not qualify for automatic caching, or (b) prepend a randomized per-session prefix to the system prompt — since OpenAI caches *leading* token sequences, a unique prefix prevents your cached prefix from matching other sessions. Note that adding a randomized suffix alone does not prevent prefix caching of the identical leading content. These are imperfect controls; the most robust mitigation for highly sensitive prompts is dedicated compute (see below), which removes shared-cache infrastructure from the picture entirely.

**Prefer dedicated compute for sensitive workloads.** Azure OpenAI's Provisioned Throughput Units and AWS Bedrock's Provisioned Throughput both provide reserved model capacity that reduces cross-tenant batch inference risk compared to shared on-demand compute, because your requests run on capacity reserved for your organization rather than competing in a shared queue. Neither provider publicly documents this as a guarantee of physically single-tenant serving infrastructure — operators with strict isolation requirements should confirm current guarantees in writing. Self-hosting via vLLM on single-tenant dedicated infrastructure is the only configuration where the isolation model is fully verifiable by the operator, at the cost of operational overhead and losing managed-service SLAs.

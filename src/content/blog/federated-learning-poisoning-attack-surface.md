...
There is no verification step. The server does not confirm that the update was computed on real data, by a legitimate device, using the prescribed training procedure. It cannot. Gradient tensors have no unforgeable provenance. An adversary who controls a client — whether by compromising a device, running a fake participant, or operating a data poisoning service — can send any gradient they like.

...
**Digest pinning**: Rather than referencing a model by name (`meta-llama/Llama-3-8B`), reference it by a specific commit hash or content digest. This gives integrity — the digest only matches the exact byte sequence that was there when you pinned it — but not authenticity (anyone could produce a file with that digest if the digest is uncompromised, though SHA-256 collisions remain impractical).

**Signed manifests**: Extending digest pinning with a Sigstore-or-similar signature gives both integrity and authenticity. A signed manifest that records the model name, the digest, the signing identity, and the timestamp means a consumer can verify not just that they got the right file, but that the right identity vouched for that file.

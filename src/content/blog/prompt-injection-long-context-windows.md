...
**Canary tokens or behavioral consistency checks.** Insert a known behavioral constraint in the system prompt — such as a specific phrase that should or should not appear in legitimate outputs, or a constraint like "always end responses with [token X]." Monitor outputs for systematic violation of that constraint, which can indicate that an injection has partially overridden the system prompt. This approach has meaningful limitations: it can leak prompt internals to users who observe the canary in outputs, train users and attackers to recognize and spoof it, and generate false alarms on normal responses. Use with care — it is one heuristic signal among several, not a standalone control, and implementation details matter significantly.

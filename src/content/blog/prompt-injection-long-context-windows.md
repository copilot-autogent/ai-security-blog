...
The security implication is not that the middle is "safe to ignore." It is that **attention is non-uniform, and an attacker who understands this can exploit it deliberately**, in either direction.
...
The "lost in the middle" effect means defenders **cannot assume that content positioned in the middle of a long context is inert**. An injection that barely fires in a 10K context can become reliably effective in a 128K context where retrieval reshuffles its position with every new query.
...
**PDF and HTML scraped content.** Documents processed by AI assistants — legal PDFs, research papers, web pages — can contain hidden instructions in comments, metadata, off-screen elements, or invisible annotations. The model processes the full serialized representation; the human sees only the rendered surface.

**Conversation history replay.** Long-context agents that maintain multi-turn conversation histories are vulnerable to a temporal injection pattern: an attacker injects malicious instructions into an early turn, knowing the conversation will eventually grow long enough that the injection shifts into an active position relative to subsequent queries. As the session accumulates context, the attacker's instruction is replayed fresh into every new turn.
...
**What makes this long-context specific.** In a 10-chunk retrieval window, the system prompt has strong influence and the injected instruction competes more directly with it. In a 50-chunk window at 128K context, the system prompt is positioned far from the most recently retrieved content and the injected instruction, benefiting from recency and semantic proximity to the user's actual query.
...
**Canary tokens in system prompt.** Insert a known phrase or token in the system prompt that should appear in legitimate outputs (or specifically not appear in outputs under certain conditions). Monitor outputs for suppression of the canary — an injection that overrides the system prompt may also suppress canary appearance. This is an imperfect heuristic, but it catches a category of full-override attacks.
...
**Retrieval ordering: minimize recency bias for untrusted sources.** If the retrieval pipeline places the most-recently-published documents at the end of the assembled context (giving them recency advantage), consider reversing the ordering for low-trust external sources. Place trusted internal documentation at the recency-advantaged end; external documents earlier in the context where they compete less with the system prompt.

...
The consequence: possession of a credential file is not sufficient to produce a valid proof. Only the device holding the secure element private key can generate one. An AI agent carrying a proof on behalf of a human can present it — but cannot forge it, cannot escalate it, and cannot reuse it across sessions (each nonce is fresh from the verifier).
...
Vega offers a third path. The human's device generates a ZKP that proves a claim about their credential, bound to the agent's session nonce. The agent carries the proof — a 108 KB opaque blob — and presents it to services. The services verify the proof cryptographically. The agent never holds the credential, cannot reuse the proof in unauthorized contexts, and cannot escalate the delegated claim beyond what the original proof committed to.
...
Vega's proof could sit directly in a browser extension or digital wallet. Age verification flows that currently require an upload could instead receive a ZKP generated in 92 ms. The verifier never sees the credential. Nothing gets stored. There's nothing to breach.
...
A Vega proof could replace the intermediary entirely. The user proves a predicate from their government credential — "I am a resident of jurisdiction X" — and the on-chain verifier receives only the proof. No third party ever sees the license or passport. The rerandomization mechanism ensures that the same credential, presented to five different smart contracts, produces five unlinkable proofs.
...
The prover key is 464 KB — fits comfortably on any phone. Proof generation: 92 ms. Proof verification: 23 ms. Proof size: 108 KB.

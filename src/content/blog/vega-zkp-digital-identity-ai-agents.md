...
**Lookup-based credential parsing** avoids building a full CBOR parser circuit. The prover instead tells the circuit "the device public key starts at byte 847" and supplies those bytes. The circuit verifies three things: the bytes match the authenticated credential, a valid CBOR field prefix appears at the claimed location, and the bytes are contiguous so the prover can't splice fields from unrelated positions. A full parser replaced by a handful of lookups. This approach works because ISO/IEC 18013-5 mandates a canonical, uniquely-encoded CBOR structure for mDL fields — ambiguous or duplicate field encodings that could confuse the byte-offset lookup don't arise in well-formed credentials that pass the issuer signature check.
...
Vega's device binding closes this. When a credential is provisioned, the issuer embeds the holder's device public key directly in the credential — tied to the phone's secure element, where the private key lives and cannot be extracted. Each proof requires the device to sign the verifier's session nonce using this private key. The ZKP circuit then extracts the device public key from the credential via the lookup mechanism and verifies that device signature over the nonce.

The consequence: possession of a credential file is not sufficient to produce a valid proof. Only the device holding the secure element private key can generate one. An AI agent carrying a proof on behalf of a human can present it — but cannot forge it, cannot escalate it, and cannot reuse it across sessions. The cross-session protection relies on verifiers issuing fresh, single-use nonces and rejecting replayed proofs — standard for well-designed verification flows, but a deployment requirement that verifiers must implement correctly.
...
Vega offers a third path. The human's device generates a ZKP bound to the verifier's challenge nonce — a fresh value issued by the service the agent is talking to. The agent carries the resulting proof — a 108 KB opaque blob — and presents it to services. The services verify the proof cryptographically. The agent never holds the credential, cannot reuse the proof in unauthorized contexts (each proof is nonce-bound to a specific verifier interaction), and cannot escalate the delegated claim beyond what the original proof committed to.
...
A Vega proof could replace the intermediary for off-chain identity assertion. The user proves a predicate from their government credential — "I am a resident of jurisdiction X" — and the on-chain verifier receives only the proof. No third party ever sees the license or passport. The rerandomization mechanism ensures that the same credential, presented to five different smart contracts, produces five unlinkable proofs. That said, on-chain verification of a 108 KB proof introduces non-trivial gas costs — practical deployment likely requires off-chain verification with on-chain attestation of the verified result, or chains with cheaper calldata costs, rather than naive on-chain proof consumption.
...
- The verifier receives the proof and learns only whether the claimed predicate is true or false about a validly-issued credential
...
If it integrates with EU Digital Identity Wallets at launch — a plausible timeline given the framework is already in motion — millions of users get privacy-preserving identity verification as a default, not an opt-in. Age verification flows, professional credential checks, and jurisdiction attestations become provable facts that don't require showing anyone the source document.

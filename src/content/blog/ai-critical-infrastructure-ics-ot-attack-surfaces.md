...
In 2022, researchers at Georgia Tech published a demonstration that should have drawn wider attention than it did. They showed that an adversary could inject carefully crafted false readings into the sensor streams of an industrial anomaly detection system — readings that, examined individually, looked entirely plausible — and cause the ML model to classify an ongoing equipment fault as normal operation. The attack was silent. The physical process was degrading. The AI watchdog said nothing was wrong.
...
The Purdue Enterprise Reference Architecture (also called the Purdue Model or ISA-95) has structured industrial network design for decades. It divides industrial systems into hierarchical levels: field devices (Level 0), control systems and PLCs (Level 1), supervisory control/SCADA (Level 2), manufacturing operations management (Level 3), and enterprise networks (Levels 4-5). The traditional security model relied on **air gaps** — physical network isolation between levels, particularly between Level 3 (where OT systems live) and Level 4+ (where IT systems and internet connectivity live).

The air gap has been eroding for over a decade. Remote access requirements for vendor maintenance, IoT connectivity, cloud analytics pipelines, and the business pressure to integrate operational data with enterprise systems have created connectivity between formerly isolated levels. The Ukraine power grid attacks in 2015 and 2016 (using the BlackEnergy and Industroyer malware families) demonstrated that air-gap erosion has real consequences: those attacks were staged through spear-phishing of IT systems, lateral movement from IT to OT networks, and ultimately manipulation of energy management systems to cause unplanned disconnections affecting 225,000 customers and 230kV substations respectively. The Ukraine incidents are the most documented cases of IT/OT-bridging attacks with physical consequences, though they attacked traditional SCADA components rather than AI systems specifically.
...
This is the attack demonstrated in the Georgia Tech research, and it generalizes across deployment contexts. An ML-based anomaly detector is trained on historical sensor data representing normal operation. At inference time, it receives a stream of sensor readings and outputs an anomaly score or classification.
...
Yan et al. (2022, "Adversarial Examples for CNN-Based Industrial Fault Detection," relevant to journals covering industrial AI security) and related research have demonstrated that adversarial perturbations of vibration sensor readings can reliably cause misclassification in CNN-based fault detection models. The perturbations can be physically realized through manipulation of the sensor signal itself or through tampering with data in transit.
...
It's important to be accurate about the current state of evidence. The documented critical infrastructure attacks with physical consequences — the Ukraine grid attacks (2015, 2016), the Saudi Aramco attack (Shamoon, 2012), the Oldsmar water treatment incident (2021), the Colonial Pipeline ransomware (2021) — were not AI-targeted attacks. They attacked traditional SCADA systems, historian databases, HMI workstations, and IT/OT network connections.
...
The ICS threat actor community is sophisticated and patient. Documented ICS intrusion campaigns — Triton/TRISIS (which targeted safety instrumented systems at a Saudi petrochemical facility in 2017, the most consequential documented ICS attack on a safety system), PIPEDREAM/INCONTROLLER (2022, a modular framework targeting multiple ICS protocols) — demonstrate that nation-state actors invest in ICS-specific capabilities, are willing to operate with long dwell times, and understand the operational specifics of industrial environments at a level that enables targeted attacks.
...
**Adversarial training**: train the anomaly detection model on adversarially perturbed sensor streams in addition to historical normal data. This is the most effective known defense against evasion attacks (Madry et al., 2017, projected gradient descent adversarial training remains the baseline), though it increases model complexity and may reduce accuracy on non-adversarial inputs. In ICS contexts, domain-specific adversarial examples — perturbations that respect physical plausibility constraints — are more useful than generic ℓ∞-bounded perturbations.

**Physics-constrained validation**: ML anomaly detectors should not operate in isolation from physics-based process models. If a sensor reading is consistent with the ML model's "normal" distribution but violates what is physically possible given adjacent sensor readings (energy balance, flow conservation, thermal dynamics), a combined system will flag what the pure ML model misses. This defense is particularly effective because physics constraints are independent of the ML model's learned features — they cannot be fooled by the same adversarial perturbations.
...
**End-to-end integrity of sensor data**: the most actionable defense against adversarial manipulation of sensor streams is ensuring cryptographic integrity from sensor to ML model. This is architecturally simple in principle but practically challenging in legacy OT environments. For new deployments, specifying authenticated measurement from intelligent field devices (IEC 61850-based protection systems, Profinet with security extensions, or OPC UA's built-in security model) closes the sensor spoofing channel at the data source. For legacy environments, data diodes or protocol gateways that perform integrity checking at the OT/IT boundary are a partial control.
...
**Air-gap LLM inference from control networks**: if LLM inference runs on cloud infrastructure, ensure the data pipeline from OT systems to the LLM is one-directional (OT → LLM, never LLM → OT). Any recommendation from the LLM travels to the operator terminal, not to a control system.

...
There is a class of AI security property that is easy to miss because it is, by design, invisible. Steganographic marking — encoding a model's identity or provenance into its output without changing semantic content — is being deployed in production AI systems right now. Claude Code appears to be among them.
...
The problem — which this post will return to — is that covert channels embedded in content are fundamentally fragile. Any transformation of the content degrades or destroys the signal.
...
**Regulated pipeline integrity**: Financial and legal workflows prohibit AI-generated content in certain positions. Code review policies at organizations handling sensitive codebases require human-written attestation. A steganographic detector keyed to known AI models offers a scalable signal for flagging submissions that bypass these controls.
...
**Effectiveness**: Near-complete against token-level marks. Partial against identifier naming (renaming variables erases naming-convention signals but modifies behavior slightly). The attacker pays a transformation cost; the output may have subtle quality differences after paraphrase.
...
**Prompt injection with forged provenance**: An adversary who understands a target organization's AI attribution scheme forges watermarks on malicious content to make it *appear* AI-generated by a trusted internal system. This is an offensive forgery use case: bypassing a "human-only" policy by impersonating the AI output that the policy trusts.
...
[^2]: thereallo.dev (2024). Claude Code Prompt Steganography. https://thereallo.dev/blog/claude-code-prompt-steganography

[^3]: Zhao, X., Ananth, P., Li, L., & Wang, Y.-X. (2023). Provable Robust Watermarking for AI-Generated Text. *International Conference on Learning Representations (ICLR 2024)*.

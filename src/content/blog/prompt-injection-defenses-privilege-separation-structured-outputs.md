...
Several LLM providers have invested in this approach. OpenAI's technical documentation describes system prompt instructions as having precedence over user messages; their model training includes explicit examples of lower-privilege content attempting to override higher-privilege instructions and being declined. Wallace et al. (2024, OpenAI) proposed instruction hierarchy as a formal training objective — published as ["The Instruction Hierarchy: Training LLMs to Prioritize Privileged Instructions"](https://arxiv.org/abs/2404.13208) — with the goal of producing models that consistently enforce privilege distinctions across tiers.
...
- **Unicode control characters**: Invisible markers that appear in the token stream but don't render visibly. These are fragile in practice: many retrieval pipelines, text normalizers, and rendering layers strip or rewrite invisible Unicode characters, so they may silently disappear before reaching the model — making them unreliable for production use. They also make prompts harder to audit and debug.
